...
Environment variables for the Supabase connection are loaded from the `.env` file at the project root. Update this file with your credentials. The frontend reads these values via `env.js`, which is generated by `scripts/generate-env.js`.
...
Update these values with your project credentials to enable API access. Run
`node scripts/generate-env.js` to create `env.js` so the frontend can access the
public values at runtime.
