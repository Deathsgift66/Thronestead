...
CREATE TABLE public.alliance_loan_repayments (
  schedule_id integer NOT NULL DEFAULT nextval('alliance_loan_repayments_schedule_id_seq'::regclass),
  loan_id integer,
...
CREATE TABLE public.alliance_votes (
  ...
  target_id integer,
  vote_metadata text,
  fallback_recovered boolean DEFAULT false,
...
CREATE TABLE public.bans (
  ...
  ip_address text,
  device_hash text,
  ban_type text NOT NULL CHECK (ban_type = ANY (ARRAY['ip'::text, 'device'::text, 'account'::text, 'other'::text])),
  reason text,
  issued_by uuid,
  ...
  CONSTRAINT bans_issued_by_fkey FOREIGN KEY (issued_by) REFERENCES public.users(user_id)
);
...
CREATE TABLE public.reauth_tokens (
  ...
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT reauth_tokens_pkey PRIMARY KEY (user_id),
  CONSTRAINT reauth_tokens_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(user_id)
);
...
CREATE TABLE public.user_reports (
  report_id integer NOT NULL DEFAULT nextval('user_reports_report_id_seq'::regclass),
  reporter_id uuid,
  target_id uuid,
  category text,
  description text,
...
CREATE TABLE public.action_queue_backup (
  backup_id integer NOT NULL GENERATED BY DEFAULT AS IDENTITY,
  source_table text NOT NULL,
  row_data jsonb NOT NULL,
  inserted_at timestamp with time zone DEFAULT now(),
  CONSTRAINT action_queue_backup_pkey PRIMARY KEY (backup_id)
);
